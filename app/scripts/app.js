...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  */
  ...
  var svgMap = function () {
    ...
    _createClass(svgMap, [{
      key: "findStation",
      value: function findStation(id) {
        console.log(id, '#scheme-layer-stations');
        console.log(this.stations[id]['labelId'], '#scheme-layer-labels');
      }
    }, {
      key: "findLabel",
      value: function findLabel(id) {
        var keyStations = Object.keys(this.stations);
        var indexStation = Object.values(this.stations).findIndex(function (item) {
      ...
      key: "findlink",
      value: function findlink(id) {
        var _this = this,
            _ref;

        var keyStations = Object.keys(this.stations);
        var indexLink = Object.values(this.stations).findIndex(function (item) {
          return item.linkIds.find(function (i) {
            return i === +id;
          });
        });
        var selectLine = Object.values(this.stations).filter(function (item) {
          return item.lineId == _this.stations[keyStations[indexLink]].lineId;
        });

        // список названий станций (текст) #scheme-layer-labels
        var selectLineIdStations = selectLine.map(function (item) {
          return item.labelId;
        });
        ...
        var linksSelect = selectLine.map(function (item) {
          return item.linkIds;
        });
        var linksSelectAll = (_ref = []).concat.apply(_ref, _toConsumableArray(linksSelect)).filter(function (elem, index, self) {
      ...
      key: "addSelectStations",
      value: function addSelectStations(id) {
        var isId = this.selectStations.findIndex(function (item) {
          return item === id;
        });
        if (isId === -1) {
      ...
      value: function cloneStation() {
        if (this.selectStations.length) {
          ...
          try {
            for (var _iterator = this.selectStations[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
              var item = _step.value;

              var station = this.$el.querySelector("#scheme-layer-stations #station-" + item).cloneNode(true);
              var label = this.$el.querySelector("#scheme-layer-labels #label-" + this.stations[item]['labelId']).cloneNode(true);
      ...
      key: "removeStation",
      value: function removeStation(id) {
        var isId = this.selectStations.findIndex(function (item) {
          return item === id;
        });
        this.selectStations.splice(isId, 1);
      ...
      value: function cloneLink(data) {
        if (data.links.length) {
          ...
          try {
            for (var _iterator2 = data.links[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
              var item = _step2.value;

              var link = this.$el.querySelector("#scheme-layer-links #link-" + item) ? this.$el.querySelector("#scheme-layer-links #link-" + item).cloneNode(true) : '';
              if (link != '') {
          ...
          var a = this.selectStations;
          var b = data.stantions;
          var c = a.concat(b);
      ...
      value: function removeLink(id) {
        ...
        for (var i = 0; i < this.selectLinks.length; i++) {
          if (this.selectLinks[i].links.findIndex(function (item) {
            return item === +id;
          }) !== -1) {
            indexLink = i;
            break;
        ...
        try {
          for (var _iterator3 = this.selectLinks[indexLink].links[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        ...
        try {
          for (var _iterator4 = this.selectLinks[indexLink].stantions[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            var _item = _step4.value;

            this.removeStation(_item);
          }
